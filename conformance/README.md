...
| `18-glyph-declarations.vs` | §2, §3.3.3, §6 | glyph declarations in the state document — a bundle carries its runes AND their meaning; declaration shadows registration; malformed descriptors refused rather than half-stored; undeclare refused while runes carry it; a declaration undoes like a rune |
| `19-kinds-and-values.vs` | §3.3.1, §3.3.2, §3.7.1 | the three rune kinds (default `entity`, nothing migrates), an act rune keeping a ternary sentence whole, `measure`/`quantity` refused on a non-measure rune, an edge weight read as an attribute's value with its unit, and `--weight` refusing a silent zero |
